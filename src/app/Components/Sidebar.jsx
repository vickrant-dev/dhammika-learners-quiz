...
import { Home, GraduationCap, Lightbulb, Menu, X } from "lucide-react"
    ...
    <>
      ...
      >
        ...
        <div id="sidebar-lists" className="flex flex-col gap-10 flex-grow">
          <div id="main-menu">
            <div className="flex justify-between items-center pb-10">
              <Image src={logo || "/placeholder.svg"} alt="logo" width={110} height={110} className={`${isMobile ? "pl-0 pt-[4.2rem]" : "pl-3.25 pt-0"}`} />
            </div>
        ...
        <div id="language-menu" className="mt-auto mb-4">
          <h4 className="select-none text-md text-primary font-semibold pb-5 pl-3.25">Choose your language</h4>
        ...
        <div className="signout mt-4">
          <button onClick={logoutDashboard} className="btn btn-warning w-full">
            Logout
          </button>
        </div>
      </div>
      ...
      {/* Overlay for mobile */}
      {isMobile && isSidebarOpen && (
        <div className="fixed inset-0 bg-white/50 filter backdrop-blur-xs z-30" onClick={() => setIsSidebarOpen(false)} />
      )}
    </>
  )
}
